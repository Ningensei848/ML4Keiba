...
local_config_username = ['git', 'config', '--local', 'user.name', GIT_AUTHOR_NAME]
...
def isProcessed(horse_id):
    ...
    filepath = DIR_LIST_HORSE / f'{horse_id[:4]}.txt' if horse_id.isdecimal() else DIR_LIST_HORSE_FOREIGN / f'{horse_id[3:7]}.txt'

    # 国産馬の場合
    if horse_id.isdecimal():
        if FILEPATH_INTERMEDIATE_HORSE.exists():
            # FILEPATH_INTERMEDIATE_HORSE を tsv として開く
            df = pd.read_csv(FILEPATH_INTERMEDIATE_HORSE, index_col=0, delimiter='\t')
    ...
    # 外国産馬の場合
        ...
        filepath = DIR_LIST_HORSE_FOREIGN / f'{horse_id[3:7]}.txt'  # list なので拡張子は .txt でよい
...
def getHorseProfile(horse_id):
    ...
    profile = {
        'horse_id': horse_id,
        'stallion': IsStallion(soup, horse_id)  # 繁殖種牡馬か判定　→　True なら getHorseSire を実行する
    }
    ...
    temp = { tr.th.text : tr.td.text.strip() for tr in db_prof_table.find_all('tr') if tr.th.text not in wasted_keys}
    profile.update(temp)
...
def getHorseResult(horse_id):
    ...
    race_id_list = [ a['href'].split('/')[2] for a in db_prof_table.find_all(href=pattern_result_href) ]

    return {
        'race_history': '-->'.join(race_id_list[::-1]) # 順番を並び替えて返して（昇順になおす），`-->` で結合
    }
...
def getHorsePedigree(horse_id):
    ...
    for gen in range(5):
        temp = df[gen].tolist()
        ancestor = [temp[i] for i in range(0, len(temp), 2 ** (4 - gen))]  # range(start, stop, step)
...
def trackAncestor(horse_id, generation):

    # 世代が5以上前，あるいはすでに処理済みである場合はスキップ
    if generation > 5 or isProcessed(horse_id):
        return
    ...
    if source.exists():
        ...
        for horse_id in set(df['horse_id'].tolist()):
            ...
            if firstCropYear not in sieve_dict:
                ...
                # 縦方向に連結
                sieve_dict[firstCropYear] = pd.concat([sieve_dict[firstCropYear], df_chunk], axis=0)
        del df
        # -----------------------------------------------------------------
        # tsv ファイルとして出力 ------------------------------------------
        ...
        for year, df in sieve_dict.items():
            ...
            if filepath.exists():
                # 縦方向に連結
                df_concat = pd.concat([ pd.read_csv(filepath, delimiter='\t'), df], axis=0)
